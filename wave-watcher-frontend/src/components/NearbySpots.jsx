...
const NearbySpots = ({
  ...
  return (
    <div
      className="mt-8 bg-transparent backdrop-blur-sm"
      style={{ background: "rgba(255, 255, 255, 0)" }}
    >
      <div className="flex items-center justify-between mb-4">
        <h3
          className="text-[10px] font-black uppercase tracking-widest"
          style={{ color: "var(--ww-text-3)" }}
        >
          Nearby Spots in this Region
        </h3>
        <div
          className="h-px flex-grow mx-4"
          style={{ background: "var(--ww-border)" }}
        ></div>
      </div>
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 xl:grid-cols-6 gap-2">
        {nearbySpots.map((spot) => (
          <div
            ...
            className="rounded-lg p-2.5 flex items-center justify-between group cursor-pointer transition-all duration-300"
            style={{
          ...
          >
            <div className="flex flex-col gap-1 min-w-0">
              <span
                className="text-[11px] font-black tracking-tight transition-colors truncate"
                style={{ color: "var(--ww-text)" }}
              >
                {spot.name}
              </span>
