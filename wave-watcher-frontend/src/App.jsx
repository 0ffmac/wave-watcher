...
const SPOTS = {
  "ujung_bocur": { name: "Ujung Bocur", lat: -5.3048, lon: 103.9919, location: "South Sumatra, Indonesia" },
  "mandiri_beach": { name: "Mandiri Beach", lat: -5.2472, lon: 103.9234, location: "South Sumatra, Indonesia" },
  "krui_left": { name: "Krui Left", lat: -5.1928, lon: 103.9290, location: "South Sumatra, Indonesia" }
};
...
function App() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const response = await fetch(`${BASE_URL}?spot=${activeSpotId}&source=stormglass&bypass_cache=true`);
  ...
  if (loading) {
    return (
      <div className="h-screen w-screen flex flex-col items-center justify-center bg-slate-50 gap-4">
        ...
        <p className="font-black text-xs uppercase tracking-[0.3em] text-slate-400">Loading Forecast</p>
      </div>
    );
  }
  ...
  const wind = data?.current?.wind || { speed: 0, direction: 0, compass: "N/A", texture: "N/A" };
    ...
    <div className="min-h-screen pb-24">
      ...
      <nav className="absolute top-0 left-0 right-0 z-50 px-6 py-6 container mx-auto flex items-center justify-between">
         <div className="flex items-center gap-6">
            <div className="flex items-center gap-2">
               ...
               <span className="font-black text-white text-xl tracking-tighter">WaveWatcher</span>
            </div>
         ...
         <div className="xl:col-span-1">
            <div className="sticky top-8 space-y-8">
               ...
               <div className="bg-gradient-to-br from-blue-600 to-indigo-700 rounded-3xl p-8 text-white shadow-2xl relative overflow-hidden group">
                  ...
                  <h4 className="font-black text-2xl mb-2 relative z-10">Go Premium</h4>
                  <p className="text-white/80 text-sm mb-6 relative z-10">Get 17-day forecasts, ad-free experience, and high-def live cams.</p>
                  <button className="bg-white text-blue-600 px-6 py-3 rounded-full font-black text-xs uppercase tracking-widest relative z-10 hover:shadow-xl transition-shadow">Learn More</button>
               </div>
            </div>
         </div>
      </div>
      ...
      <footer className="mt-24 border-t border-slate-200 bg-white/50 py-12">
        <div className="container mx-auto px-6 text-center">
           <p className="text-slate-400 font-bold text-xs uppercase tracking-[0.2em]">© 2026 WaveWatcher Forecast Services.</p>
        </div>
      </footer>
    </div>
  );
